...
exports.actualizaProyecto = async (req, res) => {
    ...
    //si el proytecto existe o no
        if(!proyecto){ return res.status(404).json({msg:'Proyecto no encontrado!'}) }

    //verificar el creador
        if(proyecto.creador.toString() !== req.usuario.id){ res.status(401).json({msg:'No autorizado'})}
...
exports.eliminaProyecto = async (req,res) =>{
    ...
    try {
        ...
        //si el proytecto existe o no
        if(!proyecto){ return res.sstatus(404).json({msg:'Proyecto no encontrado!'}) }
